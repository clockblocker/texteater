...
export function buildDumgen(options: DumgenOptions = {}): Dumgen {
	...
	const enabledGrammaticalRoutes = Object.freeze({
		de: Object.freeze({
			Lexeme: Object.freeze({
				NOUN: generators.laboratory.grammaticalResolution.de.Lexeme
					.NOUN,
			}),
		}),
	});
	...
	async function grammatical<L extends GrammaticalResolutionLanguage>(
		...
		const grammar = enabledGrammaticalRoute(
			enabledGrammaticalRoutes,
			language,
			target,
		);
		if (!grammar) {
...
function enabledGrammaticalRoute(
	routes: EnabledGrammarRoutes,
	language: "de",
	...
	| undefined {
	if (target.family !== "Lexeme" || target.kind !== "NOUN") {
		return undefined;
	}
	return routes[language].Lexeme.NOUN;
}
