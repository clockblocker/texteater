import { unitShadowSchema } from "dumrel/schema";
import { z } from "zod";

const lexemeUnitShadowSchema = z.union(
	unitShadowSchema.options.filter(
		(route) => route.shape.family.value === "Lexeme",
	),
);
const lexicalUnitShadowSchema = z.union(
	unitShadowSchema.options.filter(
		(route) => route.shape.family.value !== "Morpheme",
	),
);

import type { PromptInputSchema, PromptOutputSchema } from "promptsmith";
