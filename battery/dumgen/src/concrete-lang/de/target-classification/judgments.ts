...
Fixed correlators include only anchors, never payload: entweder/oder, weder/noch, sowohl/als/auch, nicht nur/sondern auch, je/desto are CCONJ; um/zu, ohne/zu, statt/zu, so/dass are SCONJ; einerseits/andererseits and teils/teils are ADV. Classify the whole identity, not the clicked anchor's standalone POS. A zu before an infinitive without um, ohne or statt is a singleton PART that never joins its infinitive: versucht zu schlafen gives [zu] PART and [schlafen] VERB.
