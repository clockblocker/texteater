...
Productive perfect and passive complexes are complete verbal targets: ist ... aufgefunden worden includes all three. A productive participle is VERB in every use: attributive (die gebratenen Zwiebeln, der von allen gelobte Koch, das weinende Baby), adverbial (ging pfeifend davon) and in a state passive with sein, which joins it (Das Fenster ist geöffnet gives [ist,geöffnet] VERB). An attributive or adverbial participle is a singleton VERB, and a state passive is only sein plus the participle. The participle's own objects, adverbs, agents and prepositional phrases are free material, never members, even when they decide its sense: in der von allen gelobte Koch only gelobte is the target (von, allen and Koch are Exclude), and in Das Fenster ist weit geöffnet the target is [ist,geöffnet] without weit. A participle is ADJ only when lexicalized in this sentence: its meaning here comes from no sense of its verb (spannend 'exciting', gebildet 'educated', ausgezeichnet 'excellent', erfahren 'experienced', gelegen 'convenient', verrückt 'crazy', verlegen 'embarrassed', reizend 'charming'), or it takes un- (ungelesen, unbekannt), or sehr or comparison (sehr gebildet, spannender). A participle with its own accusative object (den Hals reizend, den Bogen spannend) is always VERB. Judge the sense in this sentence, not whether the spelling has a lexicalized adjective elsewhere: a verbal complement or adjunct (a von- or vom-agent, an object, a count such as zweimal, a distance, a location) shows the verb's sense. So eine aus drei Mitgliedern gebildete Kommission (bilden), die im Tal gelegene Mühle (liegen), die gestern ausgezeichnete Autorin (auszeichnen), der den Hals reizende Staub (reizen) and das zur Seite verrückte Sofa (verrücken) are VERB, while Der Besuch kam uns sehr gelegen gives gelegen ADJ with kam a separate VERB. A lexicalized predicate keeps a separate VERB copula; substantivized participles are NOUN. bekommen, kriegen and erhalten with a Partizip II and no lexical contribution form the recipient passive: the auxiliary joins the participle's verb (bekommt ... geliefert is one VERB target). Lexical bekommen with an object, and the resultative bekommt ... geöffnet (manages to open it), keep bekommen as the VERB and leave the participle outside. Verbs that add a meaning beside a construction (sich lassen, gehören plus participle, brauchen, scheinen, drohen, versprechen or pflegen plus zu, copula bleiben) are VERB like modals. A Funktionsverbgefüge, a support verb with its predicate noun (zur Verfügung stellen, in Frage kommen, eine Entscheidung treffen, eine Frage stellen, zur Kenntnis nehmen), is one Collocation target whose members are the verb, the noun and the noun's own article or fused preposition; free arguments and adverbs stay outside (stellt den Schülern Material zur Verfügung gives [stellt,zur,Verfügung]).
