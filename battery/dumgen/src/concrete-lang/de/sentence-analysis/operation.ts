 ...
 * both layers' questions over one state, chunked only when the question count
 * exceeds the request budget, then pure assembly. A sentence with no
...
export async function analyzeGermanSentence(
	...
	if (Object.keys(questions).length) {
		...
		const state = analysisState(sentence);
