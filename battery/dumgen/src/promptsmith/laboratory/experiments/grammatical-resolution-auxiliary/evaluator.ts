...
import type {
	inputSchema,
	outputSchema,
} from "../../prompt-source/grammatical-resolution/de/lexeme/auxiliary/schemas";

export type AuxiliaryGrammaticalResolutionEvaluation = {
...
type AuxiliarySurface = NonNullable<
	output<typeof outputSchema>["resolution"]
>["surface"];

export function evaluateAuxiliaryGrammaticalResolution(args: {
...
}): AuxiliaryGrammaticalResolutionEvaluation {
	...
	const diagnostics = {
		...
		inflectionalFeaturesPass: equal(
			actualSurface?.surfaceKind === "Inflection"
				? actualSurface.inflectionalFeatures
				: null,
			expectedSurface?.surfaceKind === "Inflection"
				? expectedSurface.inflectionalFeatures
				: null,
		),
		canonicalFormPass:
...
function canonicalSurfaceFeatures(surface: AuxiliarySurface | undefined) {
