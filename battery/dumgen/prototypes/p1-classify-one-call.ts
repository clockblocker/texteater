...
async function run(
...
): Promise<CaseResult & { alt?: unknown }> {
	const calls: Call[] = [];
	const sentence = {
		id: c.id,
	...
	const project = (route: string) => {
		if (unresolvedMember || route === "Unresolved")
...
const summary = summarize(
	`P1 classify one call (unitRoute) [${process.argv[2] ?? "eval"}]`,
	results,
);
const altPass = results.filter(
	(r) => stable((r as any).alt) === stable(r.expected),
).length;
console.log(
	JSON.stringify({
		altPolicy: "singletonRoute when singleton else unitRoute",
		passed: altPass,
		passRate: +(altPass / results.length).toFixed(3),
	}),
);
