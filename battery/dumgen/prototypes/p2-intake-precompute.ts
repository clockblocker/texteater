...
const routes = {
	...
	Unresolved:
		"The unit is invalid, incomplete, includes free material, or has no defensible allowed route",
};

const scope =
	process.argv[2] === "all"
		? Object.keys(data.cases)
		: evaluationCaseIds.filter((id) => !data.demonstrationIds.includes(id));
const cases = scope.map((id) => ({
	id,
	...(data.cases as Record<string, any>)[id],
}));
...
type Analysis = {
	first: Record<number, number | "Unresolved">;
	route: Record<number, string>;
	calls: Call[];
};
async function analyse(
	...
	const resolvable = segments.flatMap((s, i) =>
		s.kind === "ResolvableText" ? [i] : [],
	);
	const state = {
		sentence: indexedContext(sentence),
		criteria:
			"In `sentence`, <sN> tags identify selectable occurrences by original segment index N. Untagged text supplies context only. Every occurrence belongs to exactly one complete fixed unit; most units have one member. " +
			targetCriteria,
	};
		...
		questions[`first_${i}`] = choice(
			`Under \`criteria\`, which occurrence in \`sentence\` is the leftmost fixed member of the complete fixed unit that contains occurrence <s${i}>? Choose Self when <s${i}> is itself the leftmost member (including when it is the only member).`,
			{
				...
				...Object.fromEntries(
					earlier.map((j) => [
						`s${j}`,
						`<s${j}> "${segments[j]!.text}" is the leftmost fixed member of the same unit as <s${i}>`,
					]),
				),
...
async function worker() {
	while (next < sentences.length) {
		...
		for (const [n, c] of group.entries()) {
			...
			let actual: unknown;
			if (r === "Unresolved" || analysis.route[k] === "Unresolved")
				actual = { decision: "Unresolved" };
			else {
				const members = Object.keys(analysis.first)
					.map(Number)
					.filter((i) => root(analysis.first, i) === r)
					.sort((a, b) => a - b);
			...
			results.push({
				...
				expected: c.idealOutput,
				actual,
				calls: n === 0 ? analysis.calls : [],
			});
		}
		if (results.length % 20 < group.length)
			console.error(`  ${results.length}/${cases.length}`);
	}
}
await Promise.all(Array.from({ length: 6 }, worker));
const summary = summarize(
	`P2 intake precompute (first-member links) [${process.argv[2] ?? "eval"}]`,
	results,
);
const routeOnly = results.filter(
	(r) =>
		r.actual &&
		(r.expected as any).kind &&
		(r.actual as any).kind === (r.expected as any).kind,
).length;
const membersOnly = results.filter(
	(r) =>
		r.actual &&
		stable((r.actual as any).memberSegmentIndices) ===
			stable((r.expected as any).memberSegmentIndices),
).length;
console.log(
	JSON.stringify({
		sentences: bySentence.size,
		routeCorrect: routeOnly,
		membersCorrect: membersOnly,
		clickTimeCalls: 0,
	}),
);
