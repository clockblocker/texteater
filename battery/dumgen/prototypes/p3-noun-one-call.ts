...
function answerFor(criteria: Record<string, unknown>, selected: string) {
	...
	return {
		...
		probabilities: Object.fromEntries(
			keys.map((k) => [k, k === pick ? 1 : 0]),
		),
	};
}

function wrappedJudge(
	segments: { kind: string; text: string }[],
	members: number[],
	calls: Call[],
): TypeSafeExecutor {
	...
	return async (request, options) => {
		...
		if (ids.includes("support")) {
				...
				extra.articleAttachment = choice(
					"Under `articlePolicy`, which complete article attachment in `sentence` is licensed for the noun target marked in `markedContext`? Judge agreement from the sentence itself.",
					{
						...Object.fromEntries(
							candidates.map((c) => [
								c.key,
								`${c.realization}: source <s${c.segmentIndex}> ${c.attested} supplies ${c.article} DET form ${c.form}. Select only if this source grammatically supplies this noun's article.`,
							]),
						),
			...
			extra.speculativeCase = featureQuestion("NOUN", casePath, {
				values: ["Nom", "Acc", "Dat", "Gen", null],
				open: false,
			});
			...
			if (variant === "head-only") {
				...
				state = {
					...state,
					markedContext: segments
						.map((s, i) =>
							i === head ? `<TARGET>${s.text}</TARGET>` : s.text,
						)
						.join(""),
					membershipNote:
						"Only the noun head is marked. Its fixed unit may also include an owned article immediately governing it; judge the whole noun phrase unit as the target.",
				};
			}
			state = {
				...state,
			...
			const result = await network(
				{
					...
					questions: { ...questions, ...(extra as Questions) },
				},
				options,
			);
			calls.push({
				route: "p3/features+article+case",
				durationMs: performance.now() - start,
				questions: ids.length + Object.keys(extra).length,
				input_tokens: result.usage?.input_tokens ?? 0,
				output_tokens: result.usage?.output_tokens ?? 0,
			});
			const answers = { ...(result.answers as Record<string, unknown>) };
			cache.attachment =
				(answers.articleAttachment as { choice?: string } | undefined)
					?.choice ?? "None";
		...
		if (ids.length === 1 && ids[0] === "attachment") {
			return {
				...
				answers: {
					attachment: answerFor(
						(
							questions.attachment as {
								criteria: Record<string, unknown>;
							}
						).criteria,
						cache.attachment ?? "Unresolved",
					),
				},
			} as never;
		}
		if (ids.length === 1 && ids[0] === casePath) {
			...
			return {
				...
				answers: {
					[casePath]: answerFor(
						(
							questions[casePath] as {
								criteria: Record<string, unknown>;
							}
						).criteria,
						cache.case ?? "Unresolved",
					),
				},
			} as never;
		}
...
async function run(id: string): Promise<CaseResult> {
	...
	const segments: { kind: string; text: string }[] = [],
		members: number[] = [];
	for (const chunk of c.input.markedContext.split(
		/(<TARGET>.*?<\/TARGET>)/gu,
	)) {
		if (!chunk) continue;
	...
	const options: DumgenOptions = {
		execute: (request) => execute(request as never) as never,
		judge: wrappedJudge(segments, members, calls),
		judgmentConfiguration: { timeoutMs: 30_000 },
	};
	const experiment = operationExperiment(
		"grammatical-resolution/de/lexeme/noun",
		options,
	);
	try {
		...
		return {
			id,
			pass: stable(output) === stable(c.idealOutput),
		...
		const unresolvedExpected =
			stable(c.idealOutput) === stable({ decision: "Unresolved" });
		return {
			id,
			pass: unresolvedExpected && /Unresolved/u.test(message),
			expected: c.idealOutput,
			actual: { decision: "Unresolved", message },
			calls,
			error: message,
		};
	}
