...
test("AUX runner preflights both frozen suites without provider access", async () => {
...
test("AUX retained evidence is strict and current-bound", () => {
	...
	expect(() =>
		parseRetainedRun({
			...valid,
			runnerVersion: "grammatical-resolution-auxiliary-v2",
		}),
	).toThrow();
	expect(() =>
