...
test("SYM runner import and preflight make no provider call", () => {
	const binding = currentEvidenceBinding();
	expect(binding.route).toBe("grammatical-resolution/de/lexeme/symbol");
...
test("SYM retained evidence is strict and current-bound", () => {
	...
	).toThrow(/obsolete evidence policy field "route"/);
...
test("SYM retained evidence preserves complete metadata for parse failures", () => {
	const valid = parseRetainedRun(draftResult());
	const firstAttempt = valid.attempts[0];
	if (firstAttempt === undefined) throw new Error("Expected a current case.");
