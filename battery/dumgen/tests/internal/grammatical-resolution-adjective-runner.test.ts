...
test("ADJ runner import and preflight make no provider call", () => {
...
test("ADJ retained evidence is strict, current-bound, and preserves errors", () => {
	const valid = parseRetainedRun(draftResult());
	expect(() => assertCurrentEvidenceBinding(valid)).not.toThrow();
	expect(() =>
		parseRetainedRun({
			...valid,
			runnerVersion: "grammatical-resolution-adjective-v2",
		}),
	).toThrow();
	expect(() =>
