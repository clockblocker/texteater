...
import {
	...
	responseRequestFor,
	summarizeEvidence,
} from "../../docs/prototypes/grammatical-resolution-other/run";
...
function passingAttempts(): RetainedAttempt[] {
	return prepareCurrentTestCases(developmentPhase).map((testCase, index) => ({
		...
		...otherGrammaticalResolutionExperiment.evaluator({
			caseId: testCase.id,
			input: testCase.input,
			idealOutput: testCase.idealOutput,
			output: testCase.idealOutput,
...
test("X shared-runner preflight is offline and phase-bound", async () => {
...
test("X requests use the shared explicit stable-prefix cache", () => {
	...
	expect(OTHER_PROMPT_CACHE_POLICY).toEqual({
		mode: "explicit",
		ttl: "30m",
...
test("X retained evidence is strict and current-bound", () => {
...
test("X finalization is offline, atomic, and recomputed", async () => {
	const directory = await mkdtemp(join(tmpdir(), "other-runner-test-"));
