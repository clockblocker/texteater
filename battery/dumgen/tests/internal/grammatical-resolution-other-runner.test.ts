...
test("X runner import and preflight make no provider call", async () => {
	expect(currentEvidenceBinding().route).toBe(
		"grammatical-resolution/de/lexeme/other",
	);
...
test("X retains complete provider metadata for parse failures", () => {
	const valid = parseRetainedRun(draftResult());
...
test("X finalization is offline, atomic, and recomputed", async () => {
	const directory = await mkdtemp(join(tmpdir(), "x-runner-test-"));
