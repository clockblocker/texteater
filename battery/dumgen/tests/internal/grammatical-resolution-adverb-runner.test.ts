...
test("ADV runner preflights both frozen suites without provider access", async () => {
...
test("ADV retained evidence is strict and current-bound", () => {
	...
	expect(() =>
		parseRetainedRun({
			...valid,
			runnerVersion: "grammatical-resolution-adverb-v2",
		}),
	).toThrow();
	expect(() =>
