...
export const comparisonInputSchema=z.union([${routes.map((_, index) => `z.strictObject({encounter:encounterSchema.options[${index}],lemma:R${index}.lemmaSchema,candidates:z.array(emojiDescriptionSchema)})`).join(",")}]);
export const knowledgeInputSchema=z.union([${routes.map((_, index) => `z.strictObject({encounter:encounterSchema.options[${index}],reading:R${index}.readingSchema,request:knowledgeRequestMaskSchema,attestedGovernment:z.array(z.strictObject({preposition:z.string().min(1),case:z.enum(["Acc","Dat","Gen"])})).optional()})`).join(",")}]);
export const grammarSchemas={${routes.map((route, index) => `${JSON.stringify(route.key)}:z.strictObject({lemma:R${index}.lemmaSchema.omit({unitKind:true,language:true,family:true,kind:true}),surface:z.strictObject(R${index}.surfaceSchema.shape).omit({unitKind:true,language:true,lemma:true,normalizedSurface:true}),normalizedMembers:z.array(z.string().min(1)).min(1),memberOrthographies:z.array(z.enum(["Standard","Typo","Fused","Shorthand"])).min(1),realizationCoverage:z.enum(["Full","Partial"]),${["de/Lexeme/VERB", "de/Lexeme/AUX", "de/Phraseme/Idiom", "de/Phraseme/Collocation"].includes(route.key) ? "expletiveEvidence:R" + index + ".attestationSchema.shape.expletiveEvidence,valencyEvidence:R" + index + ".attestationSchema.shape.valencyEvidence," : ""}${["de/Lexeme/NOUN", "de/Lexeme/PROPN"].includes(route.key) ? "articleEvidence:R" + index + ".attestationSchema.shape.articleEvidence," : ""}${["de/Lexeme/ADP", "de/Lexeme/ADJ", "de/Lexeme/NOUN"].includes(route.key) ? `valencyEvidence:R${index}.attestationSchema.shape.valencyEvidence,` : ""}})`).join(",")}};
			...
			Object.entries(schemas)
				.filter(
					([key]) =>
						...
						["intakeOutput", "knowledgeOutput"].includes(key),
				)
