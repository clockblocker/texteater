...
export const runtimePromptArtifactRecipe = defineCodegen({
	...
	build: ({ authoring }) => {
		...
		const compressedDataContent =
			brotliCompressSync(dataContent).toString("base64");
		const content = [
		...
		return [
			...
			{
				id: "dumgen-runtime-prompt-data-module",
				to: {
					target: "generated",
					path: "runtime-prompt-data.ts",
				},
				content: [
					"// Generated by codegen/generate-runtime-prompt-artifacts.ts. Do not edit.",
					"// biome-ignore format: generated runtime prompt payload",
					`export const encodedRuntimePromptData: string = ${JSON.stringify(compressedDataContent)};`,
					"",
				].join("\n"),
				provenance: authoring.map((source) => source.source),
				meta: { kind: "runtime-prompt-data-module" },
			},
		];
	},
});
