// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/SCONJ route.
...
const RUNNER_VERSION = "grammatical-resolution-subordinating-conjunction-v1";
const RUN_ROUTE = "grammatical-resolution/de/lexeme/subordinating-conjunction";
...
type Evaluation = ReturnType<
	typeof subordinatingConjunctionGrammaticalResolutionExperiment.evaluator
>;
const { inputSchema, outputSchema } =
	subordinatingConjunctionGrammaticalResolutionExperiment.promptSource;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown SCONJ Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected =
		subordinatingConjunctionGrammaticalResolutionExperiment.evaluation.ids.map(
			(id, index) => ({
				id,
				goldenCase:
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The SCONJ Grammatical Resolution suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The SCONJ Grammatical Resolution suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
