// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/PROPN route.
...
const RUNNER_VERSION = "grammatical-resolution-proper-noun-v1";
const RUN_ROUTE = "grammatical-resolution/de/lexeme/proper-noun";
...
type Evaluation = ReturnType<
	typeof properNounGrammaticalResolutionExperiment.evaluator
>;
const promptSource = properNounGrammaticalResolutionExperiment.promptSource;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown PROPN Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
async function runLiveEvaluation(): Promise<void> {
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						...
						"grammatical_resolution_proper_noun",
					),
			...
			const evaluation =
				properNounGrammaticalResolutionExperiment.evaluator({
					caseId: testCase.id,
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected =
		properNounGrammaticalResolutionExperiment.evaluation.ids.map(
			(id, index) => ({
				id,
				goldenCase:
					properNounGrammaticalResolutionExperiment.evaluation.cases[
						index
					],
			}),
		);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The PROPN suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAXIMUM_EVALUATION_CASES) {
		throw new Error(
			`The PROPN suite is capped at ${MAXIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		evaluationCaseIds: [
			...properNounGrammaticalResolutionExperiment.evaluation.ids,
		],
...
function recomputeAttemptEvaluation(attempt: RetainedAttempt): RetainedAttempt {
	...
	const evaluation = properNounGrammaticalResolutionExperiment.evaluator({
