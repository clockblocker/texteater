// PROTOTYPE ONLY — bounded live evaluation of the German Phraseme/Proverb route.
...
const RUNNER_VERSION = "grammatical-resolution-proverb-v4";
const RUN_ROUTE = "grammatical-resolution/de/phraseme/proverb";
...
type Evaluation = ReturnType<
	typeof proverbGrammaticalResolutionExperiment.evaluator
>;
const { inputSchema, outputSchema } =
	proverbGrammaticalResolutionExperiment.promptSource;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown Proverb Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
export async function runLiveEvaluation(
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						outputSchema,
						"grammatical_resolution_proverb",
					),
			...
			const evaluation = proverbGrammaticalResolutionExperiment.evaluator(
				{
					caseId: testCase.id,
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected = proverbGrammaticalResolutionExperiment.evaluation.ids.map(
		(id, index) => ({
			id,
			goldenCase:
				proverbGrammaticalResolutionExperiment.evaluation.cases[index],
		}),
	);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The Proverb Grammatical Resolution suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The Proverb Grammatical Resolution suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		evaluationCaseIds: [
			...proverbGrammaticalResolutionExperiment.evaluation.ids,
		],
