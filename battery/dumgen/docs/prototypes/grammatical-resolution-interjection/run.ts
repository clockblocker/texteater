// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/INTJ route.
...
type Evaluation = ReturnType<
	typeof interjectionGrammaticalResolutionExperiment.evaluator
>;
const promptSource = interjectionGrammaticalResolutionExperiment.promptSource;
		...
		for (const field of [
			...
			if (result[field] !== summary[field]) {
				context.addIssue({
					code: "custom",
					path: [field],
					message: `Stored evidence summary field "${field}" is inconsistent with attempts.`,
				});
			}
		}
	});
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown INTJ Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
async function runLiveEvaluation(): Promise<void> {
	...
	assertLiveApiKey(process.env.OPENAI_API_KEY);
			...
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						...
						"grammatical_resolution_interjection",
					),
...
export function assertLiveApiKey(apiKey: string | undefined): asserts apiKey {
	if (apiKey === undefined || apiKey.length === 0) {
		throw new Error(
			"OPENAI_API_KEY is unavailable; no provider client or call was created.",
		);
	}
}
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected =
		interjectionGrammaticalResolutionExperiment.evaluation.ids.map(
			(id, index) => ({
				id,
				goldenCase:
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The INTJ Grammatical Resolution suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The INTJ Grammatical Resolution suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		promptSha256: createHash("sha256")
			...
			.digest("hex"),
		evaluationCaseIds: [
			...interjectionGrammaticalResolutionExperiment.evaluation.ids,
		],
...
function recomputeAttemptEvaluation(attempt: RetainedAttempt): RetainedAttempt {
	...
	const evaluation = interjectionGrammaticalResolutionExperiment.evaluator({
