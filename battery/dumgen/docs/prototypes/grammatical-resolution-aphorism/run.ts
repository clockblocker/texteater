// PROTOTYPE ONLY — bounded live evaluation of the German Phraseme/Aphorism route.
...
const RUNNER_VERSION = "grammatical-resolution-aphorism-v7";
...
type Evaluation = ReturnType<
	typeof aphorismGrammaticalResolutionExperiment.evaluator
>;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown Aphorism Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
export async function runLiveEvaluation(
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						outputSchema,
						"grammatical_resolution_aphorism",
					),
			...
			const evaluation =
				aphorismGrammaticalResolutionExperiment.evaluator({
					caseId: testCase.id,
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected = aphorismGrammaticalResolutionExperiment.evaluation.ids.map(
		(id, index) => ({
			id,
			goldenCase:
				aphorismGrammaticalResolutionExperiment.evaluation.cases[index],
		}),
	);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The Aphorism Grammatical Resolution suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The Aphorism Grammatical Resolution suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding(
	...
	return {
		...
		evaluationCaseIds: [
			...aphorismGrammaticalResolutionExperiment.evaluation.ids,
		],
