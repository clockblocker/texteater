// PROTOTYPE ONLY — bounded live evaluation of the German Phraseme/DiscourseFormula route.
...
const RUNNER_VERSION = "grammatical-resolution-discourse-formula-v2";
const RUN_ROUTE = "grammatical-resolution/de/phraseme/discourse-formula";
...
type Evaluation = ReturnType<
	typeof discourseFormulaGrammaticalResolutionExperiment.evaluator
>;
const promptSource =
	discourseFormulaGrammaticalResolutionExperiment.promptSource;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown DiscourseFormula Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
async function runLiveEvaluation(): Promise<void> {
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						...
						"grammatical_resolution_discourse_formula",
					),
			...
			const evaluation =
				discourseFormulaGrammaticalResolutionExperiment.evaluator({
					caseId: testCase.id,
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected =
		discourseFormulaGrammaticalResolutionExperiment.evaluation.ids.map(
			(id, index) => ({
				id,
				goldenCase:
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The DiscourseFormula suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAXIMUM_EVALUATION_CASES) {
		throw new Error(
			`The DiscourseFormula suite is capped at ${MAXIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		evaluationCaseIds: [
			...discourseFormulaGrammaticalResolutionExperiment.evaluation.ids,
		],
...
function recomputeAttemptEvaluation(attempt: RetainedAttempt): RetainedAttempt {
	...
	try {
		...
		throw new Error(
			`Retained raw output for successful attempt "${attempt.caseId}" cannot be reparsed against the current output schema.`,
		);
	}
	const retainedOutput = promptSource.outputSchema.parse(attempt.output);
	if (stableJson(rawOutput) !== stableJson(retainedOutput)) {
		throw new Error(
			`Retained raw output for successful attempt "${attempt.caseId}" does not exactly match its retained output.`,
		);
	}
	const evaluation =
		discourseFormulaGrammaticalResolutionExperiment.evaluator({
