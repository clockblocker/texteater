// PROTOTYPE ONLY — bounded live evaluation of the German Construction/PairedFrame route.
...
const RUNNER_VERSION = "grammatical-resolution-paired-frame-v5";
const RUN_ROUTE = "grammatical-resolution/de/construction/paired-frame";
...
type Evaluation = ReturnType<
	typeof pairedFrameGrammaticalResolutionExperiment.evaluator
>;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown PairedFrame Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
			...
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						outputSchema,
						"grammatical_resolution_paired_frame",
					),
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected =
		pairedFrameGrammaticalResolutionExperiment.evaluation.ids.map(
			(id, index) => ({
				id,
				goldenCase:
					pairedFrameGrammaticalResolutionExperiment.evaluation.cases[
						index
					],
			}),
		);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The PairedFrame Grammatical Resolution suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The PairedFrame Grammatical Resolution suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding(
	...
	return {
		...
		evaluationCaseIds: [
			...pairedFrameGrammaticalResolutionExperiment.evaluation.ids,
		],
...
function recomputeAttemptEvaluation(attempt: RetainedAttempt): RetainedAttempt {
	...
	const evaluation = pairedFrameGrammaticalResolutionExperiment.evaluator({
