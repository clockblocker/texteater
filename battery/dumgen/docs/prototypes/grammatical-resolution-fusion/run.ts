// PROTOTYPE ONLY — bounded live evaluation of the German Construction/Fusion route.
...
const RUNNER_VERSION = "grammatical-resolution-fusion-v2";
const RUN_ROUTE = "grammatical-resolution/de/construction/fusion";
...
type Evaluation = ReturnType<
	typeof fusionGrammaticalResolutionExperiment.evaluator
>;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown Fusion Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
export async function runLiveEvaluation(
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			...
			const evaluation = fusionGrammaticalResolutionExperiment.evaluator({
				caseId: testCase.id,
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected = fusionGrammaticalResolutionExperiment.evaluation.ids.map(
		(id, index) => ({
			id,
			goldenCase:
				fusionGrammaticalResolutionExperiment.evaluation.cases[index],
		}),
	);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The Fusion Grammatical Resolution suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The Fusion Grammatical Resolution suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding(
	...
	return {
		...
		evaluationCaseIds: [
			...fusionGrammaticalResolutionExperiment.evaluation.ids,
		],
