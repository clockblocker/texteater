// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/DET route.
...
type Evaluation = ReturnType<
	typeof determinerGrammaticalResolutionExperiment.evaluator
>;
const catalogPrompt =
	PROMPT_CATALOG.laboratory.grammaticalResolution.de.Lexeme.DET.prompt;
const promptSource = determinerGrammaticalResolutionExperiment.promptSource;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown DET Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
async function runLiveEvaluation(): Promise<void> {
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						...
						"grammatical_resolution_determiner",
					),
			...
			const evaluation =
				determinerGrammaticalResolutionExperiment.evaluator({
					caseId: testCase.id,
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected =
		determinerGrammaticalResolutionExperiment.evaluation.ids.map(
			(id, index) => ({
				id,
				goldenCase:
					determinerGrammaticalResolutionExperiment.evaluation.cases[
						index
					],
			}),
		);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The DET suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The DET suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		evaluationCaseIds: [
			...determinerGrammaticalResolutionExperiment.evaluation.ids,
		],
...
function recomputeAttemptEvaluation(attempt: RetainedAttempt): RetainedAttempt {
	...
	const evaluation = determinerGrammaticalResolutionExperiment.evaluator({
