// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/AUX route.
...
const RUNNER_VERSION = "grammatical-resolution-auxiliary-v2";
// AUX has a union of Citation and verbal Inflection Surfaces. Keep enough
// response budget that provider truncation cannot masquerade as evidence.
...
type Evaluation = ReturnType<
	typeof auxiliaryGrammaticalResolutionExperiment.evaluator
>;
const catalogPrompt =
	PROMPT_CATALOG.laboratory.grammaticalResolution.de.Lexeme.AUX.prompt;
const promptSource = auxiliaryGrammaticalResolutionExperiment.promptSource;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown AUX Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
			...
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						...
						"grammatical_resolution_auxiliary",
					),
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected =
		auxiliaryGrammaticalResolutionExperiment.evaluation.ids.map(
			(id, index) => ({
				id,
				goldenCase:
					auxiliaryGrammaticalResolutionExperiment.evaluation.cases[
						index
					],
			}),
		);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The AUX suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The AUX suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		evaluationCaseIds: [
			...auxiliaryGrammaticalResolutionExperiment.evaluation.ids,
		],
...
function recomputeAttemptEvaluation(attempt: RetainedAttempt): RetainedAttempt {
	...
	const evaluation = auxiliaryGrammaticalResolutionExperiment.evaluator({
