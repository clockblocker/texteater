// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/PART route.
...
const RUNNER_VERSION = "grammatical-resolution-particle-v1";
...
type Evaluation = ReturnType<
	typeof particleGrammaticalResolutionExperiment.evaluator
>;
const { inputSchema, outputSchema } =
	particleGrammaticalResolutionExperiment.promptSource;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown PART Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
export async function runLiveEvaluation(
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						outputSchema,
						"grammatical_resolution_particle",
					),
			...
			const evaluation =
				particleGrammaticalResolutionExperiment.evaluator({
					caseId: testCase.id,
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected = particleGrammaticalResolutionExperiment.evaluation.ids.map(
		(id, index) => ({
			id,
			goldenCase:
				particleGrammaticalResolutionExperiment.evaluation.cases[index],
		}),
	);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The PART Grammatical Resolution suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The PART Grammatical Resolution suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		evaluationCaseIds: [
			...particleGrammaticalResolutionExperiment.evaluation.ids,
		],
...
function recomputeAttemptEvaluation(attempt: RetainedAttempt): RetainedAttempt {
	...
	const evaluation = particleGrammaticalResolutionExperiment.evaluator({
