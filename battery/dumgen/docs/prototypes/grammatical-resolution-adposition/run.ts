// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/ADP route.
...
const RUNNER_VERSION = "grammatical-resolution-adposition-v3";
// The ADP schema has six nullable Core Features. A 1,024-token Responses budget
// intermittently exhausts the model's reasoning allowance before it emits the
// small structured payload, so this bounded prototype records a route-local
// budget large enough to make provider truncation non-evidence.
...
type Evaluation = ReturnType<
	typeof adpositionGrammaticalResolutionExperiment.evaluator
>;
const catalogPrompt =
	PROMPT_CATALOG.laboratory.grammaticalResolution.de.Lexeme.ADP.prompt;
const promptSource = adpositionGrammaticalResolutionExperiment.promptSource;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown ADP Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			const response = await client.responses.create({
				model: catalogPrompt.generationParams.model,
				input: [
				...
				text: {
					format: zodTextFormat(
						...
						"grammatical_resolution_adposition",
					),
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected =
		adpositionGrammaticalResolutionExperiment.evaluation.ids.map(
			(id, index) => ({
				id,
				goldenCase:
					adpositionGrammaticalResolutionExperiment.evaluation.cases[
						index
					],
			}),
		);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The ADP suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The ADP suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		evaluationCaseIds: [
			...adpositionGrammaticalResolutionExperiment.evaluation.ids,
		],
...
function recomputeAttemptEvaluation(attempt: RetainedAttempt): RetainedAttempt {
	...
	const evaluation = adpositionGrammaticalResolutionExperiment.evaluator({
