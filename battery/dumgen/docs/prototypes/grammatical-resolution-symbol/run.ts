// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/SYM route.
...
const RUNNER_VERSION = "grammatical-resolution-symbol-v1";
const RUN_ROUTE = "grammatical-resolution/de/lexeme/symbol";
const REGISTRATION_MAX_OUTPUT_TOKENS = 1024;
// SYM has a union of Citation and feature-bearing Inflection Surfaces. The
// route-local live budget keeps reasoning truncation from masquerading as
// grammatical evidence while registration can retain the ordinary route cap.
...
type Evaluation = ReturnType<
	typeof symbolGrammaticalResolutionExperiment.evaluator
>;
const promptSource = symbolGrammaticalResolutionExperiment.promptSource;
...
const retainedRunSchema = z
	.strictObject({
		...
		registrationMaxOutputTokens: z.literal(REGISTRATION_MAX_OUTPUT_TOKENS),
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown SYM Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
async function runLiveEvaluation(): Promise<void> {
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			...
			const evaluation = symbolGrammaticalResolutionExperiment.evaluator({
				caseId: testCase.id,
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected = symbolGrammaticalResolutionExperiment.evaluation.ids.map(
		(id, index) => ({
			id,
			goldenCase:
				symbolGrammaticalResolutionExperiment.evaluation.cases[index],
		}),
	);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The SYM suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The SYM suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		route: RUN_ROUTE,
		model: RUN_MODEL,
		registrationMaxOutputTokens: REGISTRATION_MAX_OUTPUT_TOKENS,
		...
		evaluationCaseIds: [
			...symbolGrammaticalResolutionExperiment.evaluation.ids,
		],
