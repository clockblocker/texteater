// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/NUM route.
...
type Evaluation = ReturnType<
	typeof numeralGrammaticalResolutionExperiment.evaluator
>;
const promptSource = numeralGrammaticalResolutionExperiment.promptSource;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown NUM Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
async function runLiveEvaluation(): Promise<void> {
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						...
						"grammatical_resolution_numeral",
					),
			...
			const evaluation = numeralGrammaticalResolutionExperiment.evaluator(
				{
					caseId: testCase.id,
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected = numeralGrammaticalResolutionExperiment.evaluation.ids.map(
		(id, index) => ({
			id,
			goldenCase:
				numeralGrammaticalResolutionExperiment.evaluation.cases[index],
		}),
	);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The NUM suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAX_TEST_CASES) {
		throw new Error(
			`The NUM suite is capped at ${MAX_TEST_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		evaluationCaseIds: [
			...numeralGrammaticalResolutionExperiment.evaluation.ids,
		],
