// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/VERB route.
...
type Evaluation = ReturnType<
	typeof verbGrammaticalResolutionExperiment.evaluator
>;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown VERB Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
...
async function runLiveEvaluation(): Promise<void> {
	...
	for (const [index, testCase] of testCases.entries()) {
		...
		try {
			...
			const evaluation = verbGrammaticalResolutionExperiment.evaluator({
				caseId: testCase.id,
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected = verbGrammaticalResolutionExperiment.evaluation.ids.map(
		(id, index) => ({
			id,
			goldenCase:
				verbGrammaticalResolutionExperiment.evaluation.cases[index],
		}),
	);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The VERB suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAXIMUM_EVALUATION_CASES) {
		throw new Error(
			`The VERB suite is capped at ${MAXIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		evaluationCaseIds: [
			...verbGrammaticalResolutionExperiment.evaluation.ids,
		],
