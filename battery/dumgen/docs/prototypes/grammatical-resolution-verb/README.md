...
`grammatical-resolution/de/lexeme/verb` Prompt Source. Its Golden Corpus has 31
explicit cases: four minimized demonstrations, 20 disjoint authoritative
held-out cases, and seven corpus-only boundary or policy cases. Stable IDs do
not encode demonstration or evaluation roles.

The authoritative suite covers indicative finite, imperative, infinitive, and
participle forms; attached and detached separable prefixes; lexical
reflexivity; governed prepositions; full-verb uses of modal spellings,
`haben`, and `werden`; typo repair; and exact scope. Complementary controls
keep perfect and modal auxiliaries on AUX, agreed attributive participles on
ADJ, and reject targets that absorb an auxiliary, reflexive pronoun, governed
preposition, repeated occurrence, or unrelated verb. The split `aufpassen`
demonstration and held-out `aufstehen` control are two-member Surfaces and emit
one orthography per TARGET pair in sentence order. The held-out `sich schämen`
control independently exercises lexical reflexivity with a different Lemma and
context from the demonstration.
...
`lexicallyReflexive`, and `verbType`. Null-only Surface Features are accepted
in Structured Outputs and canonicalized to null by the evaluator.
...
- Whether modal ellipsis such as `kann Deutsch` remains AUX or becomes a modal
  VERB identity.
- Productive nominalized infinitives and the complete-member requirement for
