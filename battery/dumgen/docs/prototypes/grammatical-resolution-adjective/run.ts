// PROTOTYPE ONLY — bounded live evaluation of the German Lexeme/ADJ route.
...
const RUNNER_VERSION = "grammatical-resolution-adjective-v3";
const RUN_ROUTE = "grammatical-resolution/de/lexeme/adjective";
...
type Evaluation = ReturnType<
	typeof adjectiveGrammaticalResolutionExperiment.evaluator
>;
const promptSource = adjectiveGrammaticalResolutionExperiment.promptSource;
...
if (import.meta.main) {
	...
	if (mode === undefined) {
		...
		throw new Error(
			`Unknown ADJ Grammatical Resolution runner mode "${mode}".`,
		);
	}
}
			...
			const response = await client.responses.create({
				...
				text: {
					format: zodTextFormat(
						...
						"grammatical_resolution_adjective",
					),
...
export function prepareCurrentTestCases(): readonly PreparedTestCase[] {
	const selected =
		adjectiveGrammaticalResolutionExperiment.evaluation.ids.map(
			(id, index) => ({
				id,
				goldenCase:
					adjectiveGrammaticalResolutionExperiment.evaluation.cases[
						index
					],
			}),
		);
...
export function assertEvaluationSuiteBounds(caseCount: number): void {
	...
	if (caseCount < MINIMUM_EVALUATION_CASES) {
		throw new Error(
			`The ADJ suite requires at least ${MINIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
	if (caseCount > MAXIMUM_EVALUATION_CASES) {
		throw new Error(
			`The ADJ suite is capped at ${MAXIMUM_EVALUATION_CASES} evaluation cases; found ${caseCount}.`,
		);
	}
}
...
export function currentEvidenceBinding() {
	return {
		...
		evaluationCaseIds: [
			...adjectiveGrammaticalResolutionExperiment.evaluation.ids,
		],
...
function recomputeAttemptEvaluation(attempt: RetainedAttempt): RetainedAttempt {
	...
	const evaluation = adjectiveGrammaticalResolutionExperiment.evaluator({
