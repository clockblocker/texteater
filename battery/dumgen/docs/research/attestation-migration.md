...
| Prompt assets | All 23 route schemas/cases agree on the new occurrence/Surface shape; no semantic test expectation changes beyond ownership; generated prompts are deterministic and current. The archived Issue #22 experiment is excluded because its executable sources were removed. | `cd battery/dumgen && bun run generate:system-prompts && bun run check:system-prompts` followed by review of generated diffs |
