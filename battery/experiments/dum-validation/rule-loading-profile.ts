...
if (!destination || Bun.argv.length !== 3)
	throw new Error("Usage: bun rule-loading-profile.ts OUTPUT.json");
...
`,
	...
	for (const variant of [
		...
		"required-rules-and-reuse",
	]) {
		for (const [path, source] of originals) await writeFile(path, source);
		if (variant.startsWith("required-"))
			await applyRuleReuse(root, variant === "required-rules-and-reuse");
		let statistics = null;
		if (variant.startsWith("required-"))
		...
		for (const owner of ["dumling", "dumrel", "dumgen"]) {
			...
			if (variant.startsWith("required-") && owner === "dumgen")
				await writeFile(
					index,
			...
			if (owner === "dumgen") {
				...
				if (variant === "required-rules-and-reuse")
					await writeFile(
						index,
						(await readFile(index, "utf8")) +
							'\nexport {linguisticReuseState as __reuseState} from "./experiment-linguistic-reuse.js";\n',
					);
			}
			const entries = [
				index,
		...
		JSON.stringify(
			{
				...
				limitations:
					"Temporary rebuilds with identical explicit package externalization. Private state and parser exports are instrumentation only. Full DX gate runs separately. Encoded strings remain eagerly bundled; unit error fallbacks preserve original rules. Catalog, prompts, and model schemas are unchanged. Not deployed Convex memory.",
				results,
			},
			null,
			2,
