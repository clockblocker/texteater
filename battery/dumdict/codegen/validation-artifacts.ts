...
type _GeneratedFacadeInventoryMatchesActualSchemas = Expect<
	Equal<
...
type _DroppedFacadeLeafFails = Expect<DroppedFacadeLeafMatches>;
...
type _SwappedFacadeLeafOutputFails = Expect<SwappedFacadeLeafOutputMatches>;
...
export const dumdictValidationArtifactRecipe = defineCodegen({
	...
	build: ({ dumlingSchemas, dumdictSchemas, dumrelSchemas }) => {
		...
		const content = [
			...
			"// biome-ignore format: generated exact parser-root proof",
			"type _GeneratedRoutesMatchCanonicalSchemas = Expect<",
			...
			"\t>",
			">;",
			"type _GeneratedInternalRoutesMatchOperationalGuards = Expect<",
			"\t[",
			"\t\tGeneratedInternalDumdictValidationRouteKey,",
			"\t\tInternalDumdictOwnedValidationRouteKey,",
			"\t] extends [",
