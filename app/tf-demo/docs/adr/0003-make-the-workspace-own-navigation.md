...
Pane-local Card Layer. An already stored resolution immediately presents its
canonical Reading, Lemma, Surface, and Attestation Note subjects. During an
active Resolution, each newly available projection becomes the foremost Card
in that same ordered route while prior steps remain underneath. Completion
converges the four session-scoped Resolution Step Notes to their canonical Note
subjects; failure instead keeps the operational Resolution subject foremost.
