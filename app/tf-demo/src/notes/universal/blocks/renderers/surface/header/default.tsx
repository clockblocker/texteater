 ...
 */
export const renderDefaultSurfaceHeader = (({ noteData, PresentationCapabilities }) => {
	const count = noteData.analyses.length;
	const active = noteData.analyses.find(({ analysisKey }) => analysisKey === PresentationCapabilities.activeAnalysisKey)
	...
	return (
		<header>
			<NoteTitleRow>
				<NoteTitle data-surface-title="" tone={active ? genderTone(active.presented.lemma) : undefined}>
					{article && hasArticle ? <><NoteTitleLink
						aria-label={`${prefix}, open its DET Surface`}
						onClick={() => PresentationCapabilities.follow(article.target, article.presentationContext)}
					>{prefix}</NoteTitleLink>{form.slice(prefix.length)}</> : form}
				</NoteTitle>
