...
export async function compileRelationVerdict(
	...
	const archivedLocations = record(
		relocation.archivedArtifacts ?? {},
		"archived artifacts",
	);
	...
	for (const rawArtifact of artifacts) {
		...
		if (Object.hasOwn(archivedLocations, normalized.role)) {
			if (Object.hasOwn(locations, normalized.role))
				throw new Error(
					`Artifact is both local and archived: ${normalized.role}.`,
				);
			const archived = record(
				archivedLocations[normalized.role],
				"archived artifact",
			);
			const commit = string(archived.commit, "archive commit");
			if (!/^[a-f0-9]{40}$/.test(commit))
				throw new Error(
					"Archive reference must name a full commit hash.",
				);
			archivedArtifacts.push({
				role: normalized.role,
				commit,
				path: string(archived.path, "archived artifact path"),
	...
	if (archivedArtifacts.length > 0)
		invalidationReasons.push("archivedRunEvidence");
	...
	const compiled = {
		...
		invalidationReasons,
		verdict:
			candidateMatchesCurrentModel && archivedArtifacts.length === 0
				? compiledVerdict
				: null,
	};
