...
describe("Reading renderer inference", () => {
	...
	it("shows direct identity primitives in the editor-style hint", async () => {
		expect(
			...
			`"type GermanVerbRendererReading = { lemma: { language: "de"; family: "Lexeme"; kind: "VERB"; canonicalForm: string; coreFeatures: { hasSepPrefix: string | null; lexicallyReflexive: "Yes" | null; verbType: "Mod" | null; }; ownerKind: "Lemma"; ownerKey: string; lemmaId: Id<...>; }; emojiDescription: string; ownerKind: "Reading"; ownerKey: string; read..."`,
		);
	}, 30_000);
	...
	it("exposes only the selected language, Family, and Kind branch", async () => {
		expect(
			...
			`"type GermanVerbRendererReading = { lemma: { language: "de"; family: "Lexeme"; kind: "VERB"; canonicalForm: string; coreFeatures: { hasSepPrefix: string | null; lexicallyReflexive: "Yes" | null; verbType: "Mod" | null; }; ownerKind: "Lemma"; ownerKey: string; lemmaId: Id<"lemmas">; }; emojiDescription: string; ownerKind: "Reading"; ownerKey: string; readingId: Id<"readings">; }"`,
		);
	}, 30_000);
});
