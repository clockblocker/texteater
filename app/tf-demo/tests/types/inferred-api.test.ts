...
describe("Convex API types the client reads", () => {
	it("a Resolution Note carries its lifecycle as one tagged union", async () => {
		expect(await inferred("ResolutionNoteLifecycle")).toMatchInlineSnapshot(
			`"type ResolutionNoteLifecycle = { state: "Active"; progress: "Starting" | "RouteAvailable" | "GrammarAvailable" | "ReadingAvailable" | "Committing"; activity: "Scheduled" | "Running"; } | { state: "Terminal"; progress: "Committing"; outcome: "Complete"; attestationId: Id<"attestations">; target: { kind: "Reading"; readingId: Id<"readings">; } | { kind: "Attestation"; attestationId: Id<"attestations">; }; canonical?: { readingId: Id<"readings">; lemmaId: Id<"lemmas">; surfaceId: Id<"surfaces">; surfaceLanguage: "de"; normalizedSurface: string; attestationId: Id<"attestations">; } | undefined; } | { state: "Terminal"; progress: "Starting" | "RouteAvailable" | "GrammarAvailable" | "ReadingAvailable" | "Committing"; outcome: "Unresolved"; } | { state: "Terminal"; progress: "Starting" | "RouteAvailable" | "GrammarAvailable" | "ReadingAvailable" | "Committing"; outcome: "PermanentFailure"; failureCode: "Network" | "RateLimited" | "ProviderUnavailable" | "RequestRejected" | "InvalidOutput" | "Refusal" | "BudgetExhausted" | "CatalogMiss" | "DictionaryConflict" | "MembershipConflict" | "Internal"; diagnosticId: string; message: string; }"`,
		);
	}, 30_000);
	...
	it("a Resolution Note projects its Lemma per Kind, without any", async () => {
		expect(await inferred("VerbResolutionGrammar")).toMatchInlineSnapshot(
			`"type VerbResolutionGrammar = { grundform: boolean | null; members: { attested: string; orthography: "Standard" | "Typo"; }[]; realizationCoverage: "Partial" | "Full"; normalizedSurface: string; spelling: "Canonical" | "Variant"; canonicalForm: string; family: "Lexeme"; kind: "VERB"; coreFeatures: { hasSepPrefix: string | null; lexicallyReflexive: "Yes" | null; verbType: "Mod" | null; }; }"`,
		);
