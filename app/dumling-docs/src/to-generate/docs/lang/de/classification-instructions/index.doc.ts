...
A noun owns its article, so a German noun Surface always marks \`article\`: \`Definite\`, \`Indefinite\`, or \`None\` for a bare noun or one after a possessive, demonstrative or numeral. Put the article in the noun's \`members\` (across an adjective too: \`[Das, Band]\` in \`Das rote Band\`), set \`articleEvidence: { kind: "Owned", member }\`, and keep \`normalizedSurface\` to the noun's own letters. A fused article such as \`m\` in \`im Wald\` is an owned \`Fused\` member. Do not classify an owned article as its own \`DET\` Attestation. A \`PROPN\` owns its article the same way only if the name is canonically cited with it (\`die Schweiz\`, \`der Rhein\`, \`der Struwwelpeter\`): set the Core Feature \`article: "Definite"\` and keep the article out of \`canonicalForm\`. A name cited bare (\`Berlin\`) has \`article: null\`, and an article it takes in a sentence (\`das alte Berlin\`) is its own \`DET\` Attestation.
...
- a participle in a perfect or passive classifies as \`VERB\` with its auxiliary
- every other participle, lexicalized or not, classifies as \`ADJ\` and names its verb as \`participleSource\`
